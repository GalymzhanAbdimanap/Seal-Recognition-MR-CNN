"""
Mask R-CNN
Train on the toy Seal dataset and implement color splash effect.
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 seal.py train --dataset=/path/to/seal/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 seal.py train --dataset=/path/to/seal/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 seal.py train --dataset=/path/to/seal/dataset --weights=imagenet

    # Apply color splash to an image
    python3 seal.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>

    # Apply color splash to video using the last weights you trained
    python3 seal.py splash --weights=last --video=<URL or path to file>
"""
...
class SealDataset(utils.Dataset):
    ...
    def load_seal(self, dataset_dir, subset):
        """Load a subset of the Seal dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only one class to add.
        self.add_class("seal", 1, "seal")
    ...
    def load_image_seal(self, image_id):
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = SealDataset()
    dataset_train.load_seal(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = SealDataset()
    dataset_val.load_seal(args.dataset, "val")
    dataset_val.prepare()
...
def color_splash2(image, nns, color=(0, 255, 0), white_balance=200):
    res = []
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect seals.')
    parser.add_argument("command",
                        metavar="<command>",
                        help="'train' or 'splash'")
    parser.add_argument('--dataset', required=False,
                        metavar="/path/to/seal/dataset/",
                        help='Directory of the Seal dataset')
    ...
    if args.command == "train":
        ...
        class InferenceConfig(SealConfig):
